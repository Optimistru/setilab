...
class Server(threading.Thread):
    ...
    def run(self):
        ...
        while (True):
            ...
            elif (str(data).split(' ')[0] == 'info' and str(data).split(' ')[1].isdigit()):
            ...
            elif (str(data).split(' ')[0] == 'delete' and str(data).split(' ')[1].isdigit()):
                id = int(str(data).split(' ')[1])
                result = BaseQueries.del_by_num(id)
                if (result == 1):
                    self.socket.send("Delete successful".encode("utf-8"))
                else:
                    self.socket.send("An error occurred".encode("utf-8"))

            elif (str(data).split(' ')[0] == 'analysis' and str(data).split(' ')[1] != None):
                result = Analysis.run()

            elif (str(data).split(' ')[0] == 'stop' and str(data).split(' ')[1] != None):
